...
class Traffic:
	def __init__(self):
		self.rx = collections.deque(maxlen=5)
		self.tx = collections.deque(maxlen=5)
		self.rx.append(0)
		self.tx.append(0)
	def get(self):
		...
		for dev in net_dev[2:]:
			dev = dev.split(':')
			if dev[0].strip()[0] != "e" or dev[0].find("tun") > -1:
				continue
			dev = dev[1].split()
			avgrx += int(dev[0])
			avgtx += int(dev[8])
		...
		avgrx = int(avgrx / (l-1) / INTERVAL)
		avgtx = int(avgtx / (l-1) / INTERVAL)
...
def est_traffic():
	...
	global g_est_d
	global g_est_m
	if g_check_times>600:
		g_check_times = 0
		cmd = os.popen("vnstat -d | grep est").read().split("|")[2]
		num = re.findall(r"[0-9].*[0-9]", cmd)[0]
		if cmd.find("T") !=-1:
			unit = 1099511627776
		elif cmd.find("G") !=-1:
			unit = 1073741824
			...
			unit = 1024
		g_est_d = int(float(num)*unit)

		cmd_m = os.popen("vnstat -m | grep est").read().split("|")[2]
		num_m = re.findall(r"[0-9].*[0-9]", cmd_m)[0]
		if cmd_m.find("T") !=-1:
			unit_m = 1099511627776
		elif cmd_m.find("G") !=-1:
			unit_m = 1073741824
		elif cmd_m.find("M") !=-1:
			unit_m = 1048576
			...
			unit_m = 1024
		g_est_m = int(float(num_m)*unit_m)
	g_check_times = g_check_times + 1
	return g_est_d, g_est_m
...
def check_speed():
	...
	if check_times>600:
		check_times = 0
		out = os.popen('/root/check_status.sh').read().split("\n")
		check_status = int(out[0])
	check_times = check_times + 1	
	return int(check_status)

def get_real_load():
	return int(os.getloadavg()[0]*100)
	...
	while 1:
		try:
			...
			while 1:
				...
				array['uptime'] = Uptime
				array['load'] = Load + check_speed()*10000 + get_real_load()*100000
